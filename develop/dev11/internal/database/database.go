...
func(pg Database)EventForMonth(param models.EventDayRequest) ([]models.EventData, error){
	var EventsData []models.EventData
	query := `SELECT data_title, data_description FROM events
			WHERE user_id = $1 AND month_name = $2 AND year_number = $3`
...
func(pg Database)EventForWeek(param models.EventDayRequest, WeekNumber int) ([]models.EventData, error){
	var EventsData []models.EventData
	query := `SELECT data_title, data_description FROM events
			WHERE user_id = $1 AND week_number = $2 AND year_number = $3`
...
func(pg Database)UpdateEvent(NewEvent models.EventChange, paramDate models.Date, WeekNumber int)error{
	query := `UPDATE events
			SET user_id = $1, week_number = $2, day_number = $3, month_name = $4, year_number = $5, data_title = $6, data_description = $7
			WHERE event_id = $8`
	_, err := pg.Db.Exec(query, NewEvent.UserID, WeekNumber, paramDate.Day, paramDate.Month, paramDate.Year, NewEvent.DataEvent.Title, NewEvent.DataEvent.Description, NewEvent.EventID)
	if err != nil{
		return err
	}
	return nil
...
func(pg Database)IsUserEventExist(UserID, eventID string)(bool, error){
	...
	if err := pg.Db.QueryRow(query, eventID, UserID).Scan(&buf1, &buf2); err == constants.ErrNoRow{
		...
		log.Println(err)
		log.Println(constants.ErrNoRow)
		return false, err
	}
	return true, nil
}
